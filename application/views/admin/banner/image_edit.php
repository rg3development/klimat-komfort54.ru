	...
	<div id="content">
		<form action="<?=base_url();?>admin/banner/edit_image/<?=$data['id'];?>/<?=$parent_id;?>" method="post" enctype="multipart/form-data">
			<table class="admin_module_form">
				...
				<?php endif; ?>
				<tr><td class="admin_module_form_title">мини-изображение</td><td><input type="file" name="mini_image" size="255" />&nbsp;<strong><?= BANNER_ALT_IMAGE_TOOLTIP; ?></strong></td></tr>
				<?php if (!empty($data['mini_image']) && $data['mini_image'] != '_thumb') : ?>
				<tr><td></td><td><img src="<?=$data['path'].'/'.$data['mini_image'];?>"/></td></tr>
				<?php endif; ?>
				<tr>
