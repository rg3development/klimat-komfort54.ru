    ...
    public function add_news($parent_id = 0) {
        ...
        if ($this->form_validation->run() != FALSE) {
            if (!empty($_FILES) && $_FILES['image']['error'] == 0) {
                ...
                $small_image->createThumbnail(NEWS_IMAGE_THUMB_W, NEWS_IMAGE_THUMB_H, 'news');
            }
        ...
        $this->_view($this->_templates['news_add'], array('parent_id' => (int)$parent_id,'module_title'  => $this->_module_title,  'page_select' => $page_select));
    }
        ...
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() != FALSE) {
                if (!empty($_FILES) && $_FILES['image']['error'] == 0) {
                    ...
                    $image->createThumbnail(NEWS_IMAGE_THUMB_W, NEWS_IMAGE_THUMB_H, 'news');
                    $image_id        = $image->Save();
            ...
            $this->_view($this->_templates['news_edit'], array('parent_id' => (int)$parent_id, 'data' => $data,'module_title'  => $this->_module_title, 'page_select' => $page_select));
        } else {
            $this->_view($this->_templates['news_edit'], array('parent_id' => (int)$parent_id, 'data' => $data,'module_title'  => $this->_module_title, 'page_select' => $page_select));
        }
    }
