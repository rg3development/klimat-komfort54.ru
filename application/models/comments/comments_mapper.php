...
class Comments_mapper extends MY_Model {
    ...
    public function get_page_content($page_id = 0) {
        ...
        return $this->_view_content($template, $this->data, true);
    }
    ...
    public function get_comments_list ($approved = 0) {
        //$query = "SELECT * FROM {$this->_tables['comments']} WHERE `is_deleted` = 0 AND `approved` = {$approved} ORDER BY `page_url`";
        $query = "
            SELECT
            ...
            FROM {$this->_tables['comments']} c LEFT JOIN {$this->_tables['pages']} p ON p.url = c.page_url
