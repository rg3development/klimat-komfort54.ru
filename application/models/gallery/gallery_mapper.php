...
class Gallery_mapper extends MY_Model implements Mapper {
	...
	public function get_categories($parent_id = 0, $sort = 'desc', $sort_type = 'id') {
		...
		$sql		= "select id, parent_id, title, script_type, count_per_page from {$this->_table} {$where} order by {$sort_type} {$sort}";
	...
	public function get_category($id) {
		$sql = "select id, parent_id, title, script_type, count_per_page from {$this->_table} where id = {$id}";
	...
	public function get_all_objects($parent_id = 0, $sort = 'asc', $sort_type = 'priority') {
		$where		= $parent_id > 0 ? " where parent_id = {$parent_id}" : "";
		$sort		= $sort == 'asc' ? 'asc' : 'desc';
		$sql = "select id, parent_id, title, image_id, description, link, unix_timestamp(date_created) created from {$this->_table_item} {$where} order by {$sort_type} {$sort}";
		$data = $this->db->query($sql)->result_array();
	...
	public function get_object($id) {
		$sql = "select id, parent_id, title, image_id, description, link, unix_timestamp(date_created) created from {$this->_table_item} where id = {$id}";
		$res = $this->db->query($sql)->row_array();
		if (sizeof($res) == 0) return false;
	...
	public function get_page_content($page_id = 0) {
		...
		$config['per_page']		= $gallery_category['count_per_page'];
		...
		return $this->_view_content($template, array('gallery' => $gallery, 'offset' => $offset, 'path'=> $this->_path_to_image, 'paginator' => $paginator), true);
	}
