...
class Banner_mapper extends MY_Model implements Mapper {
	...
	public function  __construct($id = 0) {
		...
		$this->_path_to_image		=  IMAGESRC.'banner';
	...
	public function get_categories($parent_id = 0, $sort = 'desc', $sort_type = 'id') {
		...
		$sql		= "select id, parent_id, title, count_per_page from {$this->_table} {$where} order by {$sort_type} {$sort}";
	...
	public function get_category($parent_id, $type = 'parent') {
		...
		if ($type == 'parent') {
			$sql = "select id, parent_id, title, count_per_page from {$this->_table} where parent_id = {$parent_id}";
		} elseif ($type == 'object') {
			$sql = "select id, parent_id, title, count_per_page from {$this->_table} where id = {$parent_id}";
		}
	...
	public function get_all_objects($parent_id = 0, $sort = 'ASC', $sort_type = 'priority', $display = '') {
		...
		$sort		= $sort == 'ASC' ? 'ASC' : 'DESC';
		$sql = "SELECT `id`, `parent_id`, `title`, `image_id`, `mini_image_id`, `description`, `link`, `link_new_window`, `display`, unix_timestamp(`date_created`) created FROM {$this->_table_item} {$where} ORDER BY {$sort_type} {$sort}";
		$data = $this->db->query($sql)->result_array();
	...
	public function get_object($id) {
		$sql = "SELECT id, parent_id, title, image_id, mini_image_id, description, link, link_new_window, display, unix_timestamp(date_created) created from {$this->_table_item} where id = {$id}";
		$res = $this->db->query($sql)->row_array();
		if (sizeof($res) == 0) return false;
			...
			if ($object->id > 0) {
				...
				$sql = "
					...
					WHERE
						`parent_id` = {$this->db->escape($object->parent_id)}
				";
				$max = $this->db->query($sql)->row()->priority + 1;
				$created = date("Y-m-d H:i", $object->created);
	...
	public function get_widjet($id = 1) {
		$id = (int)$id;
		$banner_items = $this->get_all_objects($id, 'ASC', 'priority', 1);
		// $banner_items = array_slice($banner_items, 0, 3);
		$banner = array();
		foreach ($banner_items as $key => $item) {
			$banner[$key]['item'] = $item;
			$banner[$key]['maxi'] = new Image_item($item->image_id);
			$banner[$key]['mini'] = new Image_item($item->mini_image_id);
		}
		...
		{
			$alpha = $banner[0];
			$omega = $banner[count($banner)-1];
			array_push		($banner, $alpha); //Append first element;
			array_unshift	($banner, $omega); //Prepend last element;
		}
		*/
		return $banner;
	}
