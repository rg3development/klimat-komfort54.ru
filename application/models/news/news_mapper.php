...
class News_mapper extends MY_Model implements Mapper {
	...
	public function get_all_objects($parent_id = 0, $sort = 'desc', $sort_type = 'user_date') {
		$where		= $parent_id > 0 ? " where parent_id = {$parent_id}" : "";
		$sort		= $sort == 'asc' ? 'asc' : 'desc';
		$sql = "select id, parent_id, title, anno, description, image_id, inner_image, inner_position, unix_timestamp(date_created) created, unix_timestamp(user_date) user_date from {$this->_table_item} {$where} order by {$sort_type} {$sort}";
		$data = $this->db->query($sql)->result_array();
	...
	public function get_object($id) {
		$sql = "select id, parent_id, title, anno, description, image_id, inner_image, inner_position, unix_timestamp(date_created) created, unix_timestamp(user_date) user_date from {$this->_table_item} where id = {$id}";
		$res = $this->db->query($sql)->row_array();
		if (sizeof($res) == 0) return false;
	...
	public function get_page_content($page_id = 0) {
		...
		if ($page_id > 0 && $news_id == 0) {
			...
			$config['per_page']		= $news_category['count_per_page'];
			$config['first_link']	= 'в начало';
			$config['last_link']	= 'в конец';
			$this->pagination->initialize($config);
			$paginator = $this->pagination->create_links((int)$this->input->get('per_page'));
			$sql			= "select n.id id, n.title title, n.anno anno, n.image_id image_id, n.inner_position inner_position, unix_timestamp(n.user_date) created, i.filename filename
			...
			return $this->_view_content($this->_template['news'], array('last_news' => '', 'news_category' => $news_category, 'news_list' => $news_list, 'page_url' => $page['url'], 'page_id' => $page_id, 'offset' => $offset, 'path'=> $this->_path_to_image, 'paginator' => $paginator), true);
			...
			return $this->_view_content($this->_template['show'], array('last_news' => '', 'news' => $news, 'page_url' => $page['url'], 'page_id' => $page_id, 'offset' => $offset, 'path'=> $this->_path_to_image), true);
		}
	}

	public function get_rss($news_category_id = 0, $count = 100) {
		$sql = "select n.id id, n.title title, n.anno anno, unix_timestamp(n.user_date) created, p.url url
		...
		return $this->_view_content($this->_template['rss'], array('news_list' => $tmp, 'news_category' => $news_category, 'page_urls' => $page_urls), true);
	}

	public function get_widjet($count = 4) {
		$sql = "select n.id id, n.title title, n.anno anno, unix_timestamp(n.user_date) created, p.url url, i.filename filename
				from {$this->_table_item} n left join {$this->_table} nc on nc.id = n.parent_id left join {$this->_table_page} p on p.id = nc.parent_id
